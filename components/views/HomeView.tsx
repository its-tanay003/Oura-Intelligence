  ...
  return (
    <div className="space-y-6 md:space-y-8">
      ...
      <FadeIn direction="up" delay={0}>
          <section className="flex flex-col md:flex-row md:items-end md:justify-between border-b border-slate-100 dark:border-slate-800 pb-6 md:pb-8">
            <div>
              <h2 className="text-2xl sm:text-3xl md:text-4xl font-normal text-slate-800 dark:text-slate-100 mb-2 tracking-tight">{getGreeting()}.</h2>
            ...
            {hasData && (
                <div className="hidden md:flex flex-col items-end text-right">
                    <p className="text-sm font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wider flex items-center gap-2">
                        <PulseIndicator />
                        System Status
                    </p>
                    <p className="text-teal-600 dark:text-teal-400 font-medium">Steady Rhythm</p>
                </div>
            )}
          </section>
      </FadeIn>
            ...
            {hasData ? (
                <AmbientBreath>
                    <Card variant="default" className="relative overflow-hidden h-full">
                        ...
                        <span className="text-sm font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wider">Internal Balance</span>
                        <button 
                            onClick={() => setIsExpanded(!isExpanded)}
                            className="text-teal-600 dark:text-teal-400 text-sm md:text-base hover:underline p-2 -mr-2 rounded-lg hover:bg-teal-50 dark:hover:bg-teal-900/30 transition-colors"
        ...
        <div className="lg:col-span-5 flex flex-col gap-4">
            <FadeIn delay={2} direction="up">
                <div className="flex items-center justify-between">
                    <p className="text-sm font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wider pl-1">Recommended</p>
                </div>
                
                <Card variant="highlight" onClick={() => onChangeView(View.MIND)} className="cursor-pointer group">
                    <div className="flex items-start gap-4">
                    <div className="p-3 bg-white dark:bg-teal-900/30 rounded-full text-teal-600 dark:text-teal-400 shadow-sm dark:shadow-none shrink-0 group-hover:scale-110 transition-transform duration-300">
                        <PauseCircle size={24} />
                    </div>
                    <div>
                        <h3 className="font-medium text-slate-800 dark:text-slate-100 text-lg">Reality Check</h3>
                        <p className="text-slate-500 dark:text-slate-400 text-sm mt-1 leading-relaxed">A gentle 2-minute thought clearing exercise.</p>
                    </div>
                    <div className="ml-auto self-center text-teal-600 dark:text-teal-400 group-hover:translate-x-1 transition-transform">
                        <ArrowRight size={20} />
                    </div>
                    </div>
                </Card>
          ...
          <FadeIn delay={3} direction="up">
            <Card 
                ...
                className={`transition-all duration-500 ease-calm ${permissionGranted ? 'bg-emerald-50/50 dark:bg-emerald-900/10 border-emerald-100 dark:border-emerald-900/30' : 'cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800'}`}
            ...
            >
                ...
                <div className="flex gap-3 items-start">
                    {permissionGranted ? (
                        ...
                        <div className="mt-0.5 text-slate-400 dark:text-slate-500"><Coffee size={20} /></div>
                    )}
                    <div>
                        <span className={`font-medium block transition-colors ${permissionGranted ? 'text-emerald-800 dark:text-emerald-200' : 'text-slate-600 dark:text-slate-300'}`}>
                            {permissionGranted ? "Rest Mode Active" : "Do nothing right now"}
                        </span>
                        <span className="text-xs md:text-sm text-slate-400 dark:text-slate-500 leading-relaxed block mt-0.5">
